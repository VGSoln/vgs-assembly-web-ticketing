...
interface PumpStationMeterReadingsPageProps {
...
export const PumpStationMeterReadingsPage: React.FC<PumpStationMeterReadingsPageProps> = ({
  selectedDateRange,
  displayDateRange,
  activePreset,
  dateRangeOpen,
    ...
    try {
      switch (type) {
        case 'copy':
          ...
          break;
        case 'print':
          printData(exportData, exportHeaders, 'Pump Station Meter Readings Report');
          setExportStatus('Print dialog opened');
          break;
        case 'excel':
          await exportToExcel(exportData, exportHeaders, 'pump-station-meter-readings');
          setExportStatus('Excel file downloaded');
          break;
        case 'csv':
          exportToCSV(exportData, exportHeaders, 'pump-station-meter-readings');
          setExportStatus('CSV file downloaded');
          break;
        case 'pdf':
          await exportToPDF(exportData, exportHeaders, 'pump-station-meter-readings');
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = pumpStationMeterReadingsData.filter(reading =>
      reading.pumpStationName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      reading.pumpStationMeterNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-4 mb-6">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg px-5 py-3 text-white flex-shrink-0" style={{ width: '320px' }}>
          ...
          <div className="text-xs text-blue-100 mt-0.5">Pump Stations</div>
        </div>
        ...
        {currentData.map((reading, index) => (
          <div key={reading.id} className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-shadow duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Pump Station Name</label>
                  <p className="text-sm font-medium text-blue-600">{reading.pumpStationName}</p>
                </div>
                <div>
