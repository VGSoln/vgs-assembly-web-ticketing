...
export const TicketTypePage: React.FC<TicketTypePageProps> = ({
  ...
  const columns = [
    { key: 'ticketTypeId', label: 'Ticket Type ID', sortable: true, width: '10%' },
    { key: 'ticketTypeName', label: 'Ticket Type Name', sortable: true, width: '12%' },
    ...
    try {
      switch (type) {
        case 'copy':
          ...
          break;
        case 'print':
          printData(exportData, exportHeaders, 'Ticket Types Report');
          setExportStatus('Print dialog opened');
          break;
        case 'excel':
          await exportToExcel(exportData, exportHeaders, 'ticket-types');
          setExportStatus('Excel file downloaded');
          break;
        case 'csv':
          exportToCSV(exportData, exportHeaders, 'ticket-types');
          setExportStatus('CSV file downloaded');
          break;
        case 'pdf':
          await exportToPDF(exportData, exportHeaders, 'ticket-types');
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = ticketTypesData.filter(ticketType =>
      ticketType.ticketTypeId.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ticketType.ticketTypeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ticketType.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ticketType.createdBy.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ticketType.modifiedBy.toLowerCase().includes(searchTerm.toLowerCase())
    );
              ...
              <tbody className="bg-white">
                {currentData.map((ticketType, index) => (
                  <tr key={ticketType.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200 group ${
                    ...
                    <td className="px-1 py-2 text-10px text-slate-800 border-r border-gray-100">
                      <div className="text-10px">
                        {ticketType.modifiedDate}
                      </div>
                    </td>
                    <td className="px-1 py-2 text-10px text-slate-800 border-r border-gray-100">
                      <span className="text-10px">
                        {ticketType.modifiedBy}
                      </span>
                    </td>
            ...
            <div key={ticketType.id} className="border-b border-gray-200 p-4 hover:bg-gray-50">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <span className="text-sm font-semibold ml-1">{ticketType.ticketTypeId}</span>
                </div>
                <button 
                  onClick={() => handleEditTicketType(ticketType)}
              ...
              <div className="text-sm text-blue-600 font-semibold mb-1">{ticketType.ticketTypeName}</div>
              <div className="text-xs text-gray-600 mb-2">{ticketType.description}</div>
              <div className="mb-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <span className="ml-1">{ticketType.createdDate}</span>
                </div>
                <div>
                  <span className="text-gray-500">By:</span>
                  <span className="ml-1">{ticketType.createdBy}</span>
                </div>
                <div>
                  <span className="text-gray-500">Modified:</span>
                  <span className="ml-1">{ticketType.modifiedDate}</span>
                </div>
                <div>
                  <span className="text-gray-500">By:</span>
                  <span className="ml-1">{ticketType.modifiedBy}</span>
                </div>
              </div>
            </div>
          ))}
