...
interface CustomerMeterReadingsPageProps {
...
export const CustomerMeterReadingsPage: React.FC<CustomerMeterReadingsPageProps> = ({
  selectedDateRange,
  displayDateRange,
  activePreset,
  dateRangeOpen,
  ...
  const columns = [
    ...
    { key: 'staffName', label: 'Staff', sortable: true, width: '9%' },
    ...
    try {
      switch (type) {
        case 'copy':
          ...
          break;
        case 'print':
          printData(exportData, exportHeaders, 'Customer Meter Readings Report');
          setExportStatus('Print dialog opened');
          break;
        case 'excel':
          await exportToExcel(exportData, exportHeaders, 'customer-meter-readings');
          setExportStatus('Excel file downloaded');
          break;
        case 'csv':
          exportToCSV(exportData, exportHeaders, 'customer-meter-readings');
          setExportStatus('CSV file downloaded');
          break;
        case 'pdf':
          await exportToPDF(exportData, exportHeaders, 'customer-meter-readings');
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = customerMeterReadingsData.filter(reading =>
      reading.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      reading.customerNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
      reading.customerPhone.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      reading.zone.toLowerCase().includes(searchTerm.toLowerCase()) ||
      reading.customerType.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  return (
    <div className="space-y-3">
      {/* Top Dropdowns Row - Note: This has 4 dropdowns including Zone */}
      ...
      <div className="flex gap-4 mb-6">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg px-5 py-3 text-white flex-shrink-0" style={{ width: '320px' }}>
          ...
          <div className="text-xs text-blue-100 mt-0.5">Customers</div>
        </div>
                ...
                {currentData.length === 0 ? (
                  ...
                  currentData.map((reading, index) => (
                    <tr key={reading.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200 group ${
                      ...
                      <td className="px-3 py-3 text-11px border-r border-gray-100">
                        <a 
                          href="#" 
                          onClick={(e) => {
                            e.preventDefault();
                            console.log('Customer # clicked:', reading.customerNumber);
                          }}
                          className="text-blue-600 hover:text-blue-800 break-all leading-tight transition-colors duration-200 hover:underline"
                        >
                          {reading.customerNumber}
                        </a>
                      </td>
                      <td className="px-3 py-3 text-11px text-slate-800 border-r border-gray-100">
                        <button 
                          onClick={() => console.log('Customer Name clicked:', reading.customerName)}
                          className="break-words leading-tight font-medium text-blue-600 hover:text-blue-800 hover:underline transition-colors duration-200 cursor-pointer w-full text-left"
                        >
                      ...
                      <td className="px-3 py-3 text-11px text-slate-800 border-r border-gray-100 text-center">
                        <div className="leading-tight">
                          <div className="break-words">
                            <span className="font-semibold">{reading.serverDate.split(' ').slice(0, 4).join(' ')}</span>
                            {reading.serverDate.split(' ').length > 4 && (
                              <span className="text-11px text-slate-500 ml-1">{reading.serverDate.split(' ').slice(4).join(' ')}</span>
                            )}
                          </div>
                        </div>
                      </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div>
                  ...
                  <button 
                    onClick={() => console.log('Customer Name clicked:', reading.customerName)}
                    className="block text-sm font-medium text-blue-600 hover:text-blue-800 hover:underline transition-colors w-full text-left"
                  >
                    {reading.customerName}
                  </button>
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-700">{reading.customerPhone}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700 font-mono bg-gray-100 px-2 py-1 rounded">{reading.meterNumber}</p>
                </div>
                <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-900">{reading.reading}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Field Reading</label>
                  <p className="text-sm font-semibold text-gray-900">{reading.fieldReading}</p>
                </div>
              </div>
            </div>
          </div>
