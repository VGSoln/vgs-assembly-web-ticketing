...
import { paidCustomersData, oneMonthOwedData, twoMonthsOwedData, threeMonthsOwedData } from '@/lib/customerDebtDataByMonth';
...
interface DashboardDetailsCustomerDebtPageProps {
...
export const DashboardDetailsCustomerDebtPage: React.FC<DashboardDetailsCustomerDebtPageProps> = ({
  selectedDateRange,
  displayDateRange,
  activePreset,
  dateRangeOpen,
  ...
  const [monthsOwedFilter, setMonthsOwedFilter] = useState<number | null>(null);

  // Check for filter in sessionStorage on mount
  React.useEffect(() => {
    const filterStr = sessionStorage.getItem('customerDebtFilter');
    if (filterStr) {
      const filter = JSON.parse(filterStr);
      if (filter.monthsOwed !== undefined) {
        setMonthsOwedFilter(filter.monthsOwed);
      }
      sessionStorage.removeItem('customerDebtFilter');
    }
  }, []);
  ...
  // Select appropriate data based on filter
  const getDataByFilter = () => {
    if (monthsOwedFilter === 0) return paidCustomersData;
    if (monthsOwedFilter === 1) return oneMonthOwedData;
    if (monthsOwedFilter === 2) return twoMonthsOwedData;
    if (monthsOwedFilter === 3) return threeMonthsOwedData;
    return customerDebtData; // 4+ months (existing data)
  };
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    let filtered = baseData.filter(debt => {
      // Apply search filter
      return debt.customerNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return filtered;
  }, [searchTerm, sortConfig, monthsOwedFilter]);
