...
interface PumpStationsPageProps {
...
export const PumpStationsPage: React.FC<PumpStationsPageProps> = ({
  selectedDateRange,
  displayDateRange,
  activePreset,
  dateRangeOpen,
  ...
  const [showAddPumpStation, setShowAddPumpStation] = useState(false);
  ...
  // Navigation handlers
  const handleAddPumpStation = () => {
    setShowAddPumpStation(true);
  };

  const handleBackFromAdd = () => {
    setShowAddPumpStation(false);
  };

  const handleSavePumpStation = (pumpStationData: any) => {
    console.log('Saving pump station:', pumpStationData);
  ...
  const handleSaveEdit = (pumpStationData: any) => {
    console.log('Saving edited pump station:', pumpStationData);
    // Here you would implement the actual save logic
    setShowEdit(false);
    setShowDetails(false);
    setSelectedStation(null);
    ...
    try {
      switch (type) {
        case 'copy':
          ...
          break;
        case 'print':
          printData(exportData, exportHeaders, 'Pump Stations Report');
          setExportStatus('Print dialog opened');
          break;
        case 'excel':
          await exportToExcel(exportData, exportHeaders, 'pump-stations');
          setExportStatus('Excel file downloaded');
          break;
        case 'csv':
          exportToCSV(exportData, exportHeaders, 'pump-stations');
          setExportStatus('CSV file downloaded');
          break;
        case 'pdf':
          await exportToPDF(exportData, exportHeaders, 'pump-stations');
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = pumpStationsData.filter(station =>
      station.waterSystemName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      station.pumpStationName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      station.meterNumber.includes(searchTerm) ||
  ...
  const averageDaysSinceLastReading = Math.round(
    pumpStationsData.reduce((sum, station) => sum + station.daysSinceLastReading, 0) / pumpStationsData.length
  );

  // Show Add Pump Station page if state is true
  if (showAddPumpStation) {
    return (
      <AddPumpStationPage 
        onBack={handleBackFromAdd}
        onSave={handleSavePumpStation}
      />
    );
  }

  // Show Edit page if state is true
  if (showEdit && selectedStation) {
    return (
      <EditPumpStationPage 
        pumpStationData={selectedStation}
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-2 text-white">
          ...
          <div className="text-xs text-blue-100">Total Pump Stations</div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder="Search pump stations, comments..."
            />
          </div>
        </div>
      </div>
            ...
            <table className="w-full table-fixed border-collapse min-w-[800px]">
              ...
              <tr>
                {columns.map((column) => (
                  <th 
                    ...
                    } ${column.key === 'pumpStationNumber' ? 'rounded-tl-xl' : ''} ${column.key === 'actions' ? 'rounded-tr-xl' : ''}`}
              ...
              <tbody className="bg-white">
                {currentData.map((station, index) => (
                  <tr key={station.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200 group ${
                    ...
                    <td className="px-2 py-2 text-11px text-slate-800 border-r border-gray-100">
                      <div className="leading-tight text-center">
                        <div className="break-words font-semibold text-10px">{station.lastReadingDate}</div>
                      </div>
                    </td>
                    <td className="px-2 py-2 text-11px text-slate-800 border-r border-gray-100 text-center">
                      <span className="text-11px font-semibold text-slate-800">
                        {station.daysSinceLastReading}
                      </span>
                    </td>
                    <td className="px-2 py-2 text-11px text-slate-800 border-r border-gray-100 text-center">
                      <span className="text-10px font-semibold text-slate-800">
                        {station.lastReading.toLocaleString()}
                      </span>
                    </td>
      ...
      <div className="md:hidden space-y-4">
        ...
        {currentData.map((station, index) => (
          <div key={station.id} className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-shadow duration-200">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{station.pumpStationNumber}</h3>
                <p 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-900">{station.throughput}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Meter #</label>
                  <p className="text-sm text-gray-700 font-mono bg-gray-100 px-2 py-1 rounded">{station.meterNumber}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{station.lastReadingDate}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Days Since Last Reading</label>
                  <p className="text-sm font-semibold text-gray-900">{station.daysSinceLastReading}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Last Reading</label>
                  <p className="text-sm font-semibold text-gray-900">{station.lastReading.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
