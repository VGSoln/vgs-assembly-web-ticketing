...
export const CustomerTypePage: React.FC<CustomerTypePageProps> = ({
  ...
  const handleSaveCustomerType = (customerTypeData: any) => {
    console.log('Saving customer type:', customerTypeData);
    setShowAddCustomerType(false);
  };

  const handleEditCustomerType = (customerType: any) => {
    setSelectedCustomerType(customerType);
    setShowEditCustomerType(true);
  };

  const handleBackFromEdit = () => {
    setShowEditCustomerType(false);
    setSelectedCustomerType(null);
  };

  const handleSaveEdit = (customerTypeData: any) => {
    console.log('Saving edited customer type:', customerTypeData);
    setShowEditCustomerType(false);
    setSelectedCustomerType(null);
  };
  ...
  // Show Add Customer Type page if state is true
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-2 text-white">
          ...
          <div className="text-xs text-blue-100">Total Customer Types</div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
            <table className="w-full table-fixed border-collapse min-w-[1000px]">
              ...
              <tbody className="bg-white">
                {currentData.map((customerType, index) => (
                  <tr key={customerType.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200 group ${
      ...
      <div className="md:hidden">
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          {currentData.map((customerType) => (
            <div key={customerType.id} className="border-b border-gray-200 p-4 hover:bg-gray-50">
